...
export const getAgenteById = (req, res) => {
  const agente = getConnectionAgentes().data.agentes.find(
    (agente) => agente.id == req.params.id
  );
...
export const getAgenteByCodigo = (req, res) => {
  ...
  const filteredAgentes = agentes.filter(
    (agente) =>
      agente.codigo === codigoNumber || String(agente.codigo).includes(codigo)
  );
...
export const updateAgente = async (req, res) => {
  ...
  let agente = getConnectionAgentes().data.agentes.find(
    (agente) => agente.id == req.params.id
  );
  if (!agente) return res.sendStatus(404);
  ...
  getConnectionAgentes().data.agentes.map((t) =>
    t.id == req.params.id ? agente : t
  );
...
export const deleteAgente = (req, res) => {
  ...
  if (!agente) return res.sendStatus(404);
  const newdata = getConnectionAgentes().data.agentes.filter(
    (agente) => agente.id != req.params.id
  );
